...
def cal_annual_per_area_unit(
    monthly_per_area_unit,
):  # unit: g/m2/year - for plot global map
...
def cal_monthly_ds(ds, model_name):  # unit: Tg/month - for plot trend
    for f in SFLTF_LIST:
        ds_sftlf = xr.open_dataset(f) if model_name in f else 1
    for f in AREA_LIST:
        ds_area = xr.open_dataset(f) if model_name in f else 1

    reindex_ds_lf = (
        ds_sftlf[VAR_SFTLF].reindex_like(ds[var_name], method="nearest", tolerance=0.01)
        * 1e-2
    )
    reindex_ds_area = ds_area[VAR_AREA].reindex_like(
        ds[var_name], method="nearest", tolerance=0.01
    )
    monthly_ds = (
        ds[var_name].transpose(..., "time")
        * reindex_ds_lf
...
def plot_global_seasonal_trend(seasonal_ds, mode="abs"):
    ...
    global_rate_anml = (
        global_rate.groupby("time.month") - global_rate.groupby("time.month").mean()
    )
...
var_area = xr.open_dataset(
    "../data/axl/areacella/areacella_fx_CESM2-WACCM_historical_r1i1p1f1_gn.nc"
)


def cal_mon_ds(ds):  # unit: Tg/month
    reindex_ds_var = ds["loadoa"].reindex_like(
        var_area["areacella"], method="nearest", tolerance=0.01
    )
